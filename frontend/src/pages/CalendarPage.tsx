...
import { addDays, formatDate, localIsoDate, parseDurationInput, startOfWeek, weekDates } from '../lib/format'
...
function PlannedSessionForm({ defaultDate, onSaved }: { defaultDate: string; onSaved: () => void }): React.JSX.Element {
  ...
  return <form className="stack-form" onSubmit={(event) => void submit(event)}>
    ...
    <label className="switch-row"><input type="checkbox" checked={form.is_locked} onChange={(event) => setForm((current) => ({ ...current, is_locked: event.target.checked }))} /><span><strong>Lock this session</strong><small>Weekly planning will preserve this session unchanged.</small></span></label>
