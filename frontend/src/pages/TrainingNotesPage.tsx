...
export function TrainingNotesPage(): React.JSX.Element {
  ...
  return <div className="page notes-page">
    <PageHeader eyebrow="KNOWLEDGE — NOT EVIDENCE" title="Training Notes" description="Capture useful ideas without letting external content silently rewrite your programme." actions={<Button icon="plus" onClick={() => setEditorOpen(true)}>New note</Button>} />
...
function NoteReader({ note }: { note: TrainingNote }): React.JSX.Element {
  ...
  return <article className="note-reader">
    ...
    <div className={`coaching-state ${note.use_for_coaching ? 'enabled' : ''}`}><Icon name={note.use_for_coaching ? 'check' : 'info'} /><div><strong>{note.use_for_coaching ? 'Approved for coaching context' : 'Knowledge only'}</strong><p>{note.use_for_coaching ? 'This note may inform AI context, but measured athlete evidence still takes precedence.' : 'This note cannot change coaching or plans.'}</p></div></div>
    {note.actionable_ideas.length > 0 && <Button variant="ghost" icon="target" onClick={() => setPrincipleOpen(true)}>Create coaching principle</Button>}
    <Modal open={principleOpen} title="Create coaching principle" onClose={() => setPrincipleOpen(false)}><PrincipleForm note={note} onSaved={() => setPrincipleOpen(false)} /></Modal>
  </article>
}
